...
class Bowtie2(Index):
    ...
    def get_path(self, fontools_path_main, species_abv, division, release, genome_version_std, suffix):
        return os.path.join(
            fontools_path_main, "bowtie2_indices", f"{species_abv}_genome_all_{division}_{genome_version_std}{suffix}"
        )

    def get_create_cmds(self, path_genome, path_gff, path_genome_chrom_length, num_processor):
        return [["bowtie2-build", "--threads", str(num_processor), path_genome, "seq"]]
...
class Star(Index):
    ...
    def get_path(self, fontools_path_main, species_abv, division, release, genome_version_std, suffix):
        return os.path.join(
            fontools_path_main,
            "star_indices",
            f"{species_abv}_genome_all_cdna_all_{division}{release}_{genome_version_std}{suffix}_sjdboverhang75",
        )
    ...
    def get_create_cmds(self, path_genome, path_gff, path_genome_chrom_length, num_processor):
        ...
        cmds.append(
            [
                ...
                "75",
                "--genomeSAindexNbases",
                str(star_string_length(get_genome_length(path_genome_chrom_length))),
                "--runThreadN",
                str(num_processor),
            ]
        )
...
idx2classes = {"bowtie2": Bowtie2, "star": Star}
