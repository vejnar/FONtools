...
def open_table(path_tsv, index_col=None, index_integer=True, names=None, name="Table", list_out=True, filters=None):
    if names is None:
        names = ["id"]
...
class EnsemblDBGO:
    def __init__(
        self, path_table_transcript, path_table_object_xref, path_table_xref, path_table_ontology, path_table_term
    ):
        # Source
        source = ensembl.Ensembl()
        # Open species tables
        self.table_transcript = open_table(
            path_table_transcript,
            index_col="stable_id",
            index_integer=False,
            names=source.table_transcript_names,
            list_out=False,
        )
        self.table_object_xref = open_table(
            path_table_object_xref,
            index_col="ensembl_id",
            names=source.table_object_xref_names,
            filters=[["ensembl_object_type", "Transcript"]],
        )
        self.table_xref = open_table(
            path_table_xref,
        ...
        self.table_ontology = open_table(
            path_table_ontology,
            names=source.table_ontology_names,
            filters=[["name", "GO"]],
            list_out=False,
        )
        self.table_term = open_table(
            path_table_term,
            index_col="accession",
            index_integer=False,
