...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        action="store",
        required=True,
        help="Path to annotation file(s) (comma separated).",
    )
    parser.add_argument(
        "-o",
        "--output",
        ...
        action="store",
        required=True,
        help="Path to output file (comma separated).",
    )
    parser.add_argument(
        "-t",
        "--output_format",
        ...
        action="store",
        default="fon",
        help="Output format: fon or fon1.",
    )
    parser.add_argument(
        "-e",
        "--output_feature",
    ...
    parser.add_argument(
        ...
        dest="data_source",
        action="store",
        help="Data source: ensembl, or xenbase.",
    )
    parser.add_argument(
        "-b",
        "--biotype",
        ...
        action="store",
        default="all",
        help="Biotype: all, protein_coding (comma separated).",
    )
    parser.add_argument(
        "-f",
        "--fasta",
    ...
    parser.add_argument(
        ...
        dest="cdna",
        action="store_true",
        help="FASTA file containing cDNA.",
    )
    parser.add_argument(
        "-u",
        "--ucsc_names",
        dest="convert_ucsc_names",
        action="store_true",
        help="Convert to UCSC chromosome/contig names.",
    )
    parser.add_argument(
        "-m",
        "--path_mapping",
        dest="path_mapping",
        action="store",
        help="Path to chromosome name mapping.",
    )
    parser.add_argument(
        "-l",
        "--filter",
        dest="source_filter",
        action="append",
        help="Filter source (ex: ensembl, havana).",
    )
    parser.add_argument(
        "-x",
        "--exclude_no_seq",
        dest="exclude_no_seq",
        action="store_true",
        help="Exclude transcript without cDNA sequence.",
    )
    parser.add_argument(
        "-z",
        "--compress",
    ...
    parser.add_argument(
        ...
        dest="bed_name_as_id",
        action="store_true",
        help="BED: Use name (BED 4th column) as ID.",
    )
    parser.add_argument(
        "--bed_feature_id",
        dest="bed_feature_id",
        action="store",
        default="transcript_stable_id",
        help="BED: Feature ID (default:transcript_stable_id).",
    )
    parser.add_argument(
        "--bed_interval_name",
        dest="bed_interval_name",
        action="store",
        default="exons",
        help="BED: Interval name (default:exons).",
    )
    parser.add_argument(
        "--table_transcript",
        dest="path_table_transcript",
        action="store",
        help="Path to table transcript.txt.gz",
    )
    parser.add_argument(
        "--table_object_xref",
        dest="path_table_object_xref",
        action="store",
        help="Path to table object_xref.txt.gz",
    )
    parser.add_argument(
        "--table_xref",
        dest="path_table_xref",
        action="store",
        help="Path to table xref.txt.gz",
    )
    parser.add_argument(
        "--table_ontology",
        dest="path_table_ontology",
        action="store",
        help="Path to table ontology.txt.gz",
    )
    parser.add_argument(
        "--table_term",
        dest="path_table_term",
        action="store",
        help="Path to table term.txt.gz",
    )
    ...
    if args.path_annot.find(".gff3") != -1:
        assembly, transcripts = ft.gff.get_transcripts_gff3(
            args.path_annot.split(","), args.convert_ucsc_names, args.path_mapping, args.data_source, args.source_filter
        )
    elif args.path_annot.find(".bed") != -1:
        transcripts = ft.bed.get_transcripts_bed6(
            args.path_annot.split(","),
            args.convert_ucsc_names,
            args.path_mapping,
            args.bed_name_as_id,
            args.bed_feature_id,
            args.bed_interval_name,
        )
    else:
    ...
    if (
        args.path_table_transcript
        and args.path_table_object_xref
        and args.path_table_xref
        and args.path_table_ontology
        and args.path_table_term
    ):
        logger.info("Add gene ontology (GO)")
        db = ft.ensembl_db.EnsemblDBGO(
            args.path_table_transcript,
            args.path_table_object_xref,
            args.path_table_xref,
            args.path_table_ontology,
            args.path_table_term,
        )
