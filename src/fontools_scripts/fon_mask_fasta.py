...
def main(argv=None):
    ...
    parser.add_argument(
        "-x",
        "--exterior_extension",
        dest="exterior_extension",
        action="store",
        type=int,
        default=-1,
        help="Extension for exterior boundaries of features.",
    )
    parser.add_argument(
        "-l",
        "--seq_length",
        ...
        type=int,
        default=1000,
        help="Length of sequence line(s)",
    )
    parser.add_argument(
        "-r",
        "--inverse",
        ...
        action="store_true",
        default=False,
        help="Inverse mask (parts in input FON are masked)",
    )
    ...
    with open(args.output_fasta, "wb", buffering=1024 * 1024 * 12) as fout:
        for name, seq in gfasta.records.items():
            ...
            ntot += len(mseq)
    logger.info(f"Unmasked sequence: {nunmsk}nt ({nunmsk / ntot * 100.0:.1f}%)")
